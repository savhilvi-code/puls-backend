...
def _build_links_from_topics(topics: Any) -> list[dict]:
...
def _build_forums_found(topics: Any) -> list[str]:
    if not isinstance(topics, list):
        return []
    forums: list[str] = []
    for item in topics:
...
async def parse_diagnostic(router_json: dict) -> dict:
    ...
    payload = {
        "query": query,
        "lang": router_json.get("language", "en"),
        "car_info": router_json.get("active_car") or router_json.get("car_info") or "",
        "conversation_history": router_json.get("conversation_history") or "",
    ...
    try:
        async with httpx.AsyncClient(timeout=45, follow_redirects=True) as client:
            ...
            if response.status_code in {404, 405} and not endpoint.rstrip("/").endswith("/search"):
                ...
                response = await client.post(alternate, json=payload)
