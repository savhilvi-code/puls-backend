...
- 2026-07-18: Tightened full-VIN lookup to avoid fabricated matches. The backend VIN cascade now stops at confirmed PULS data, free NHTSA decoding, and local WMI/year decoding, while open web search remains only for JDM chassis research.
- 2026-07-18: Expanded the JDM local chassis dictionary with real user examples (`PNT30`, `E11`, `NZE124`, `GS131`) and widened compact chassis parsing so one-letter prefixes like `E11...` are recognized instead of being rejected as unknown.
